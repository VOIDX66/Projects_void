...
void loop() {
  ...
  if (bandera_ejecucion){
    ...
    if(event_temp.temperature-5 > 24){
      SerialBT.print("Calor");
      delay(10000);
    }
    else if(gyro.gyro.y > 0.5){
      SerialBT.print("Ingreso");
    }
    else
    {
      Serial.printf("Temperature: %.2f °C  Humidity: %.0f\%%\nAccel X: %.2f   Y: %.2f   Z: %.2f  m/s^2\nGyro X: %.2f   Y: %.2f   Z: %.2f  radians/s \n"
      ...
      SerialBT.printf("Temperature: %.2f °C  Humidity: %.0f\%%\n"
                    ,event_temp.temperature-5,event_humi.relative_humidity);              
    }
